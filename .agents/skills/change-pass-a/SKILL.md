...
name: change-pass-a
description: Scope-change step 2: change impact analysis → change-impact-report.md; get_slash_command: change-pass-a
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/change-pass-a`** (plural `commands`), **or**
...
- Retry with **`get_slash_command`** and `change-pass-a`.
