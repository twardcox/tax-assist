...
name: pre-flight
description: Lint, format, tests; MCP ce://commands/pre-flight
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/pre-flight`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`pre-flight`**
...
- Retry with **`get_slash_command`** and `pre-flight`.
