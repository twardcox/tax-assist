...
name: pr-description
description: PR description markdown; MCP ce://commands/pr-description
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/pr-description`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`pr-description`**
...
- Retry with **`get_slash_command`** and `pr-description`.
