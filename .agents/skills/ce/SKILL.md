...
name: ce
description: CE skills help - lists all available Agent Skills like --help; loads catalog from ce://skills
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/ce`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`ce`**
...
- Retry with **`get_slash_command`** and `ce`.
