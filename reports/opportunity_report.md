# UTBIS Opportunity Report

**Generated:** 2026-06-05 14:44  
**Tax Year:** 2025  
**Benefits Evaluated:** 58  
**Opportunities Found:** 37  

---


## Eligible Now

### Energy Efficient Home Improvement Credit (§25C)
- **Status:** `eligible_now`
- **Category:** energy
- **Estimated Value:** $600 – $3,200/year
- **Risk Level:** low
- **Forms:** Form 5695, Part II

§25C Energy Efficient Home Improvement Credit: 30% credit on eligible improvements. Heat pump: up to $2,000/year; insulation, windows, doors, energy audit: up to $1,200/year ($3,200 combined cap).

**Next Steps:**
- Get ENERGY STAR or §25C certification from manufacturer before purchase
- Heat pump HVAC or water heater = up to $2,000 30% credit
- Insulation, exterior doors, windows = up to $1,200 30% credit
- Home energy audit = up to $150 toward $1,200 cap
- Spread improvements across multiple years to use the $3,200 annual cap each year

---
### Annual Gift Tax Exclusion
- **Status:** `eligible_now`
- **Category:** estate
- **Estimated Value:** $19,000–$38,000 per recipient per year removed from taxable estate
- **Risk Level:** low
- **Forms:** Form 709 (only required if above annual exclusion or gift splitting)
- **CPA Review:** recommended

Annual gift tax exclusion: $19,000 per recipient per year (2025) — $38,000 per recipient for married couples.

**Next Steps:**
- Identify recipients: children, grandchildren, siblings, etc.
- Make gifts by December 31
- Direct tuition/medical payments to institutions are additionally excluded (no dollar limit)
- Consider 529 superfunding: 5 years of exclusion at once ($95,000 single / $190,000 MFJ per beneficiary)

---
### Business Vehicle Deduction (Mileage or Actual Expense)
- **Status:** `eligible_now`
- **Category:** business_deduction
- **Estimated Value:** ~$5,628+ per year
- **Risk Level:** low
- **Forms:** Schedule C, Part IV (vehicle information), Form 4562 (if claiming depreciation)

Business vehicle: 8,400 miles × $0.67 = ~$5,628 (standard mileage).

**Next Steps:**
- Ensure contemporaneous mileage log exists (date, destination, purpose)
- Compare standard mileage vs. actual expense method
- If vehicle GVWR > 6,000 lbs: evaluate Section 179 deduction

---
### Charitable Contribution Deduction
- **Status:** `eligible_now`
- **Category:** individual_deduction
- **Estimated Value:** $500 – $50,000+/year depending on giving levels
- **Risk Level:** low
- **Forms:** Schedule A (itemized deduction reporting), Form 8283 (non-cash charitable contributions over $500)

Charitable contribution deduction available (itemizing confirmed).

**Next Steps:**
- Donate appreciated stock directly instead of cash — avoid capital gains AND get full FMV deduction
- Document all contributions — written acknowledgment required for gifts over $250

---
### Child and Dependent Care Credit (CDCC)
- **Status:** `eligible_now`
- **Category:** individual_credit
- **Estimated Value:** ~$600/year
- **Risk Level:** low
- **Forms:** Form 2441 (Child and Dependent Care Expenses)

Child and Dependent Care Credit: ~$600 (20% of $3,000 qualifying expenses).

**Next Steps:**
- Report on Form 2441
- Collect care provider TIN (EIN or SSN)
- Note: Dependent Care FSA ($3,000) reduces CDCC expense base

---
### Child Tax Credit (CTC)
- **Status:** `eligible_now`
- **Category:** individual_credit
- **Estimated Value:** ~$4,000/year
- **Risk Level:** low
- **Forms:** Schedule 8812 (Credits for Qualifying Children and Other Dependents)

Child Tax Credit: 2 qualifying child(ren) × $2,000 = ~$4,000.

**Next Steps:**
- Report on Schedule 8812
- Up to $1,700 per child is refundable (ACTC)

---
### Mortgage Interest Deduction
- **Status:** `eligible_now`
- **Category:** individual_deduction
- **Estimated Value:** ~$17,800/year deduction
- **Risk Level:** low
- **Forms:** Schedule A (Itemized Deductions)

Mortgage interest ~$17,800 deductible if itemizing.

**Next Steps:**
- Collect Form 1098 from lender
- Report on Schedule A

---
### Qualified Opportunity Zone Investment
- **Status:** `eligible_now`
- **Category:** investment
- **Estimated Value:** Deferred tax on $6,500 + potential 10-year exclusion on QOF appreciation
- **Risk Level:** moderate
- **Forms:** Form 8949 (original gain reporting), Form 8997 (QOF investment tracking)
- **CPA Review:** recommended

Capital gains of ~$6,500 recorded — Opportunity Zone investment would defer this tax.

**Next Steps:**
- Identify and invest in a Qualified Opportunity Fund (QOF) within 180 days of gain recognition
- Note: deferred gain recognized December 31, 2026 — plan for that tax event
- 10+ year hold permanently excludes QOF appreciation from income

---
### Qualified Business Income (QBI) Deduction
- **Status:** `eligible_now`
- **Category:** business_deduction
- **Estimated Value:** ~$4,300/year (before taxable income cap)
- **Risk Level:** low
- **Forms:** Form 8995 (below income threshold), Form 8995-A (above income threshold or multiple businesses)
- **CPA Review:** recommended

QBI deduction available — estimated 20% of ~$21,500 net profit.

**Next Steps:**
- Use Form 8995 if below income thresholds
- Confirm business is not a Specified Service Trade (SSTB)
- Model home office and retirement contribution impact on QBI

---
### State and Local Tax Deduction (SALT)
- **Status:** `eligible_now`
- **Category:** individual_deduction
- **Estimated Value:** Up to $10,000/year
- **Risk Level:** low
- **Forms:** Schedule A (Itemized Deductions)
- **CPA Review:** recommended

SALT deduction available (capped at $10,000). Property tax: $8,200/year. Evaluate state PTE tax election to bypass the SALT cap on pass-through income.

**Next Steps:**
- Report state income tax + property tax on Schedule A (combined cap $10,000)
- If self-employed in high-tax state: ask CPA about Pass-Through Entity (PTE) tax election

---
### Section 121 Primary Residence Gain Exclusion
- **Status:** `eligible_now`
- **Category:** real_estate
- **Estimated Value:** Up to $500,000 gain excluded
- **Risk Level:** low
- **Forms:** Schedule D (Capital Gains and Losses), Form 8949 (Sales and Other Dispositions of Capital Assets)
- **CPA Review:** recommended

Section 121 exclusion available — up to $500,000 gain excluded on home sale. Estimated gain ~$153,000 is fully within exclusion.

**Next Steps:**
- Track all capital improvements to increase basis
- Document rental period (if any) — depreciation recapture applies

---
### State Homestead Property Tax Exemption
- **Status:** `eligible_now`
- **Category:** real_estate
- **Estimated Value:** $200 – $2,000+/year
- **Risk Level:** low
- **Forms:** County homestead exemption application (varies by county)

Homestead exemption already applied in WA. If you have a spouse, senior, or veteran status, check for enhanced exemptions.

**Next Steps:**
- Verify the exemption amount on your property tax statement
- Check for senior/veteran enhanced exemptions if applicable

---
### No State Income Tax Benefit
- **Status:** `eligible_now`
- **Category:** individual_credit
- **Estimated Value:** $2,000 – $30,000+/year depending on income and prior-state comparison
- **Risk Level:** low

WA has no broad-based state income tax — you owe $0 in state income tax on wages, self-employment income, and most other income.

**Next Steps:**
- Ensure your domicile is established in your state (driver's license, voter registration, bank address)
- Part-year residents: confirm no tax owed to prior state for the portion of year lived there
- Community property states (TX, NV, WA): review federal planning interactions with your CPA

---

## Nearly Eligible (Small Gap)

### American Opportunity Tax Credit (AOTC)
- **Status:** `nearly_eligible`
- **Category:** education
- **Estimated Value:** $1,000 – $2,500/year per eligible student
- **Risk Level:** low
- **Forms:** Form 8863 (Education Credits)

Has dependents — confirm if any are enrolled in first 4 years of college.


**Missing Information:**
- `dependents.education.school_level`
- `dependents.education.tuition_paid`

---
### Bonus Depreciation (First-Year Expensing)
- **Status:** `nearly_eligible`
- **Category:** business_deduction
- **Estimated Value:** $2,000 – $200,000+/year depending on purchases
- **Risk Level:** low
- **Forms:** Form 4562 (must elect OUT of bonus depreciation if not desired)
- **CPA Review:** recommended

Bonus depreciation available (40% in 2025) on qualifying assets placed in service. Rate drops to 20% in 2026.

**Next Steps:**
- Identify qualifying asset purchases this year
- Apply Section 179 first, bonus on remainder
- Consider cost segregation study on real estate for QIP reclassification

---
### Residential Clean Energy Credit (Solar, Battery, Wind)
- **Status:** `nearly_eligible`
- **Category:** energy
- **Estimated Value:** $3,000 – $15,000+ per installation
- **Risk Level:** low
- **Forms:** Form 5695 (Residential Energy Credits)

Homeowner qualifies for 30% credit on solar panels, battery storage, wind, or geothermal installed at your home.

**Next Steps:**
- Get solar quotes from 3+ installers — 30% credit applies through 2032
- Battery storage (3 kWh+) qualifies even without solar
- Check state and utility rebates that stack on top of federal credit

---
### Employer-Provided Childcare Credit (§45F)
- **Status:** `nearly_eligible`
- **Category:** business
- **Estimated Value:** $5,000 – $150,000/year depending on childcare spending
- **Risk Level:** low
- **Forms:** Form 8882 (Credit for Employer-Provided Childcare Facilities and Services), Form 3800 (General Business Credit) — attach Form 8882
- **CPA Review:** recommended

Has a business — §45F credit is available if you pay for qualified childcare facilities or resource/referral services for employees. No W-2 employees recorded yet.


**Missing Information:**
- `businesses.employees.w2_employees_count`

---
### Clean Vehicle Credit (EV Tax Credit)
- **Status:** `nearly_eligible`
- **Category:** energy
- **Estimated Value:** $3,750 – $7,500 (new); $2,000 – $4,000 (used)
- **Risk Level:** low
- **Forms:** Form 8936 (Clean Vehicle Credits)

Income qualifies for EV credit (up to $7,500 new / $4,000 used). Check vehicle eligibility at fueleconomy.gov.

**Next Steps:**
- Verify vehicle VIN qualifies at fueleconomy.gov before purchasing
- Use point-of-sale transfer option to receive credit as immediate discount at dealer
- Check MSRP limits: SUV/truck/van ≤ $80,000; sedan ≤ $55,000

---
### Health Savings Account (HSA) — Triple Tax Advantage
- **Status:** `nearly_eligible`
- **Category:** healthcare
- **Estimated Value:** $700 – $3,500+/year in tax savings; $100k+ lifetime value
- **Risk Level:** low
- **Forms:** Form 8889 (annual HSA reporting), Schedule 1, Line 13 (deduction for contributions not through payroll)

Not confirmed on HDHP. Switching to a qualifying HDHP unlocks the HSA triple tax advantage.


**Missing Information:**
- `healthcare.hdhp_enrolled`

**Changes Needed:**
- Switch to a High Deductible Health Plan (min deductible $1,650 self / $3,300 family for 2025)
- Open HSA account at Fidelity (no fees) after HDHP enrollment

---
### ICHRA / QSEHRA — Individual Health Reimbursement Arrangement
- **Status:** `nearly_eligible`
- **Category:** healthcare
- **Estimated Value:** $2,000 – $12,800/employee/year in tax-free health reimbursements
- **Risk Level:** low
- **Forms:** Form W-2 Box 12 Code FF (QSEHRA reimbursements — employer reports), Form 8962 (employees claiming PTC must reduce by QSEHRA amount)
- **CPA Review:** recommended

Has a business — ICHRA/QSEHRA allows tax-free health reimbursements to employees as an alternative to group health insurance. No W-2 employees recorded yet.


**Missing Information:**
- `businesses.employees.w2_employees_count`

---
### Net Unrealized Appreciation (NUA) — §402(e)(4)
- **Status:** `nearly_eligible`
- **Category:** retirement
- **Estimated Value:** $10,000 – $500,000+ lifetime depending on appreciation and tax bracket
- **Risk Level:** medium
- **Forms:** Form 1040 Line 5b (taxable retirement distributions — ordinary income portion), Schedule D (LTCG when employer stock is eventually sold)
- **CPA Review:** recommended

Has retirement plan — if plan holds appreciated employer stock, NUA strategy may apply. Record employer stock NUA amount to calculate potential savings.


**Missing Information:**
- `retirement.employer_plans.traditional_401k.employer_stock_nua`

---
### Section 179 Immediate Expensing
- **Status:** `nearly_eligible`
- **Category:** business_deduction
- **Estimated Value:** $5,000 – $400,000+/year depending on purchases
- **Risk Level:** low
- **Forms:** Form 4562 (Depreciation and Amortization — Section 179 claimed here)
- **CPA Review:** recommended

Has business — Section 179 available if equipment or vehicles are purchased this year.

**Next Steps:**
- Record any business assets purchased in businesses.yaml
- Consider whether needed equipment purchase makes sense before year-end

**Missing Information:**
- `businesses.depreciation.assets_placed_in_service`

---
### SEP-IRA Contribution Deduction
- **Status:** `nearly_eligible`
- **Category:** retirement
- **Estimated Value:** Up to $4,995 deductible contribution
- **Risk Level:** low
- **Forms:** Schedule C (net profit calculation), Schedule 1, Line 16 (self-employed SEP deduction), No separate IRS filing — report on Form 1040
- **CPA Review:** recommended

SEP-IRA not yet established. Can contribute up to $4,995 for tax year 2025.

**Next Steps:**
- Open SEP-IRA at Fidelity, Vanguard, or Schwab (takes 15 minutes)
- Can establish and fund up to October 15 (with extension)
- Max contribution: $4,995

---
### Small Employer Retirement Plan Startup Credit (§45E)
- **Status:** `nearly_eligible`
- **Category:** retirement
- **Estimated Value:** $500 – $5,000/year for 3 years ($1,500 – $15,000 total startup credit)
- **Risk Level:** low
- **Forms:** Form 8881 (Credit for Small Employer Pension Plan Startup Costs)
- **CPA Review:** recommended

Has a business — §45E credit is available when you add employees and set up a new retirement plan. No W-2 employees recorded (solo operators do not qualify for this specific credit).


**Missing Information:**
- `businesses.employees.w2_employees_count`

---
### Solo 401(k) — Self-Employed Retirement Plan
- **Status:** `nearly_eligible`
- **Category:** retirement
- **Estimated Value:** Up to $28,464 in combined contributions
- **Risk Level:** low
- **Forms:** Schedule C or W-2 (profit/compensation basis), Schedule 1, Line 16 (deductible contributions), Form 5500-EZ (annual if assets > $250,000)
- **CPA Review:** recommended

Solo 401(k) not yet established. Must be set up by December 31 to contribute for 2025.

**Next Steps:**
- Open Solo 401(k) at Fidelity (free plan, no admin fees)
- MUST be established by December 31 — cannot retroactively create
- Employee deferrals also due by December 31
- Employer profit-sharing contribution can be made up to October 15

---
### Work Opportunity Tax Credit (WOTC) — §51
- **Status:** `nearly_eligible`
- **Category:** business
- **Estimated Value:** $2,400 – $9,600 per qualifying hire; more for disabled veterans
- **Risk Level:** low
- **Forms:** Form 5884 (Work Opportunity Credit), Form 3800 (General Business Credit) — attach Form 5884, Form 8850 (Pre-Screening Notice — filed with state agency, not IRS)

Has a business — WOTC is available when you hire from targeted groups (veterans, SNAP recipients, ex-felons, long-term unemployed, etc.). No W-2 employees recorded yet.


**Missing Information:**
- `businesses.employees.w2_employees_count`

---
### State Pass-Through Entity (PTE) Tax Election
- **Status:** `nearly_eligible`
- **Category:** business_deduction
- **Estimated Value:** $3,000 – $50,000+/year
- **Risk Level:** moderate
- **Forms:** CA Form 3893 (PTE Elective Tax Payment Voucher), NY IT-204-LL / IT-653, NJ PTE-100 / PTE-150, Federal Schedule E / K-1 (entity-level tax flows through)
- **CPA Review:** recommended

Residence state WA has not enacted a PTE election. If this business operates in other states, add them as operating_states — you may have a PTE opportunity in a nexus state.


**Missing Information:**
- `businesses.businesses[*].operating_states`

---
### County Homestead Property Tax Exemption
- **Status:** `nearly_eligible`
- **Category:** real_estate
- **Estimated Value:** $50 – $500+/year
- **Risk Level:** low
- **Forms:** County homestead exemption application (varies by county — search '[county] homestead exemption application')

You own a primary residence and likely qualify for United States County, WA's county homestead exemption. Most counties administer their own exemption on top of the state exemption, but it is not automatic — you must apply with the county assessor.

**Next Steps:**
- Search 'United States County, WA homestead exemption application' to find the county assessor portal
- Gather deed/mortgage statement + government ID showing current address
- File before the county deadline (most states: March 1)
- Confirm you also have the state-level exemption — both layers are required separately

---
### County Solar / Renewable Energy Property Tax Exemption
- **Status:** `nearly_eligible`
- **Category:** real_estate
- **Estimated Value:** $150 – $1,500+/year (varies with system size, county rate, and home value)
- **Risk Level:** low
- **Forms:** County renewable energy exemption application (varies — search '[county] solar property tax exemption')

WA mandates that counties exempt the added value of solar installations from property tax assessment. If you have or are considering solar panels, their value will not increase your property tax bill.

**Next Steps:**
- Verify your current property tax bill does not include solar panel value
- In mandatory-exemption states this is typically automatic after installation
- Stack with the federal 30% Residential Clean Energy Credit (Form 5695)
- Factor this exemption into your solar ROI calculation before installing

---

## Eligible If You Make Changes

### 529-to-Roth IRA Rollover (SECURE 2.0)
- **Status:** `eligible_if_changed`
- **Category:** education
- **Estimated Value:** Up to $35,000 in Roth IRA contributions without the usual income limit
- **Risk Level:** low
- **Forms:** Form 5329 (if applicable, for excess contributions), Roth IRA custodian rollover documentation

SECURE 2.0 §126: unused 529 funds can roll to the beneficiary's Roth IRA tax-free. No 529 account recorded. Open one now to start the 15-year account age requirement.

**Next Steps:**
- Open a 529 plan today — even a small balance starts the 15-year clock
- Contributions made within the last 5 years cannot be rolled over
- Lifetime limit: $35,000 per beneficiary; annual Roth IRA limit applies

**Changes Needed:**
- Open a 529 account — the account must be at least 15 years old before rolling

---
### Augusta Rule — Home Rental to Business (Section 280A(g))
- **Status:** `eligible_if_changed`
- **Category:** business_deduction
- **Estimated Value:** $5,000 – $25,000/year (14 days × fair market daily rate)
- **Risk Level:** moderate
- **Forms:** No Form required for homeowner (income not reported on tax return), Business Schedule C or S Corp return deducts rent as ordinary business expense
- **CPA Review:** recommended

Augusta Rule available — rent your home to your business for up to 14 days/year tax-free.


**Changes Needed:**
- Schedule legitimate business meeting(s) or events at your home
- Create a written rental agreement between yourself and your business
- Invoice your business at fair market rental rate (document comparable rates)
- Ensure payment is actually made from business account to your personal account
- Document the meeting agenda and attendees
- Keep total rental days at 14 or fewer — the 15th day eliminates the exclusion

---
### Conservation Easement Deduction — §170(h)
- **Status:** `eligible_if_changed`
- **Category:** real_estate
- **Estimated Value:** $50,000 – $1M+ lifetime deduction for owners of qualifying land
- **Risk Level:** high
- **Forms:** Form 8283 Section B (Noncash Charitable Contributions > $500), Schedule A Line 12 (noncash charitable contributions)
- **CPA Review:** recommended

Has real estate — conservation easement deduction (§170(h)) requires land with conservation potential (farm, ranch, undeveloped land, habitat, scenic corridor). No qualifying land type identified in your data.

**Next Steps:**
- CAUTION: Only pursue with a reputable land trust — not a promoter
- Syndicated easements are IRS listed transactions with heavy penalties

**Changes Needed:**
- Own land with qualifying conservation purpose

---
### Installment Sale Method — §453
- **Status:** `eligible_if_changed`
- **Category:** real_estate
- **Estimated Value:** $5,000 – $100,000+ in lifetime tax savings from bracket spreading
- **Risk Level:** low
- **Forms:** Form 6252 (Installment Sale Income) — filed every year payments are received, Schedule D (capital gain summary)
- **CPA Review:** recommended

Has a business — installment sale method available if you sell the business and negotiate seller financing with the buyer.

**Next Steps:**
- Model tax impact with CPA before agreeing to sale terms

**Changes Needed:**
- Negotiate installment terms in business sale agreement

---
### S Corporation Election (SE Tax Reduction)
- **Status:** `eligible_if_changed`
- **Category:** entity_election
- **Estimated Value:** $3,000 – $20,000+/year
- **Risk Level:** moderate
- **Forms:** Form 2553 (S Corp election — must file within 2 months 15 days of tax year start), Form 941 (quarterly payroll tax deposits), Form W-2 (annual wage reporting for owner), Form 1120-S (S Corp annual return), Schedule K-1 (owner's share of income)
- **CPA Review:** recommended

Net profit ~$21,500 — S Corp payroll overhead (~$600–1,200/year) may offset savings at this level.


**Changes Needed:**
- Grow net profit above ~$50,000 where S Corp savings clearly exceed costs
- Model: SE tax savings vs. payroll costs at current profit level

---

## Future Opportunities

### Net Operating Loss (NOL) Carryforward
- **Status:** `future_opportunity`
- **Category:** business_deduction
- **Estimated Value:** Depends on NOL amount — 37% federal rate × carryforward balance
- **Risk Level:** low
- **Forms:** Form 1045 (Application for Tentative Refund — optional; used for carryback), Form 1040 Schedule 1 (other income/loss), IRS Publication 536 worksheet (to compute NOL)
- **CPA Review:** recommended

Business is profitable (net $21,500). NOL carryforward becomes relevant in any future loss year. Track cumulative NOL balance if prior years had losses.

**Next Steps:**
- Review prior returns — any year with net loss may have created an unused NOL carryforward

---
### Qualified Longevity Annuity Contract (QLAC) — §401(a)(9)
- **Status:** `future_opportunity`
- **Category:** retirement
- **Estimated Value:** $2,000 – $20,000/year in deferred RMDs and reduced current tax
- **Risk Level:** low
- **Forms:** No special form — excluded from RMD balance calculation automatically, Form 1040 Line 5b (when annuity payments begin, taxed as ordinary income)
- **CPA Review:** recommended

QLAC is most valuable near or in retirement. At age 42, focus on maximizing contributions first. Revisit at age 60+.

**Next Steps:**
- Maximize IRA/401k contributions now to grow the balance that funds a QLAC later

---
### Qualified Small Business Stock Exclusion (§1202)
- **Status:** `future_opportunity`
- **Category:** investment
- **Estimated Value:** Can exclude $10 million to $100 million+ in gains (federal tax eliminated)
- **Risk Level:** high_review_required
- **Forms:** Form 8949 (Sales and Other Dispositions of Capital Assets), Schedule D
- **CPA Review:** recommended

If you invest in or found a qualifying C corporation startup, §1202 may exclude 100% of gains up to $10M+. Set investments.has_qualified_small_business_stock: true if applicable.

**Next Steps:**
- For startup founders: early exercise of options + §83(b) election starts the 5-year holding period
- Ensure company is a C corp (not LLC or S corp) with assets ≤ $50M at time of investment
- Document original issuance (not secondary purchase) to preserve §1202 eligibility

---

_This report is for planning purposes only. Consult a licensed CPA or tax attorney before implementing any strategy._
