...
name: merge-pr
description: Verify CI + reviews, merge open PR (squash/merge/rebase), Tracker → Done, switch to main; MCP ce://commands/merge-pr
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/merge-pr`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`merge-pr`**
...
- Retry with **`get_slash_command`** and `merge-pr`.
