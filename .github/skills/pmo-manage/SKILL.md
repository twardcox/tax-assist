...
name: pmo-manage
description: PMO management session: orient, gap and drift analysis, next-command menu; get_slash_command: pmo-manage
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/pmo-manage`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`pmo-manage`**
...
- Retry with **`get_slash_command`** and `pmo-manage`.
