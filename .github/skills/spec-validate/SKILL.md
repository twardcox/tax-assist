...
name: spec-validate
description: Validate spec completeness and AC coverage; get_slash_command: spec-validate
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/spec-validate`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`spec-validate`**
...
- Retry with **`get_slash_command`** and `spec-validate`.
