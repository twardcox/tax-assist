...
name: commit
description: Conventional commit (auto sequential-commit if 15+ paths); MCP ce://commands/commit
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/commit`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`commit`**
...
- Retry with **`get_slash_command`** and `commit`.
