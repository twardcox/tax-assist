...
description: Milestone progress report from Tracker snapshot; get_slash_command: milestone-status
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/milestone-status`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`milestone-status`**
...
- Retry with **`get_slash_command`** and `milestone-status`.
