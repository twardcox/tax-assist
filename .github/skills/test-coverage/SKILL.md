...
name: test-coverage
description: Test coverage workflow; MCP ce://commands/test-coverage
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/test-coverage`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`test-coverage`**
...
- Retry with **`get_slash_command`** and `test-coverage`.
