...
description: Milestone mid/end: Tracker vs Milestone PRD, update spec, flag extension; get_slash_command: milestone-review
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/milestone-review`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`milestone-review`**
...
- Retry with **`get_slash_command`** and `milestone-review`.
