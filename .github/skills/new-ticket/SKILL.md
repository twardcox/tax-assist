...
name: new-ticket
description: Create task/bug (quick) or user story (full change workflow: pass-a + pass-b → change-apply); MCP ce://commands/new-ticket
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/new-ticket`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`new-ticket`**
...
- Retry with **`get_slash_command`** and `new-ticket`.
