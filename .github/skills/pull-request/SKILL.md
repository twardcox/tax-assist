...
name: pull-request
description: After pre-PR gate + ai-review, push, PR; MCP ce://commands/pull-request
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/pull-request`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`pull-request`**
...
- Retry with **`get_slash_command`** and `pull-request`.
