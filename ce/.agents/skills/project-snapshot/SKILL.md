...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/project-snapshot`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`project-snapshot`**
...
- Retry with **`get_slash_command`** and `project-snapshot`.
