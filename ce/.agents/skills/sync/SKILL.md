...
name: sync
description: Pull framework bundle and overwrite repo files (apply first; diff optional); MCP ce://commands/sync
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/sync`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`sync`**
...
- Retry with **`get_slash_command`** and `sync`.
