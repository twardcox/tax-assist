...
description: Sprint wrap-up: retro + carry-over + next sprint prep; get_slash_command: sprint-complete
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/sprint-complete`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`sprint-complete`**
...
- Retry with **`get_slash_command`** and `sprint-complete`.
