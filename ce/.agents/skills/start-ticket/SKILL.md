...
name: start-ticket
description: Ticket lists then 3 Qs (ticket, self/agent, branch); Next Steps + auxiliary; MCP ce://commands/start-ticket
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/start-ticket`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`start-ticket`**
...
- Retry with **`get_slash_command`** and `start-ticket`.
