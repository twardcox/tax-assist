...
name: update-config
description: Update .ce-project.json; MCP ce://commands/update-config
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/update-config`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`update-config`**
...
- Retry with **`get_slash_command`** and `update-config`.
