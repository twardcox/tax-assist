...
name: ai-review
description: Pre-PR review; MCP ce://commands/ai-review
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/ai-review`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`ai-review`**
...
- Retry with **`get_slash_command`** and `ai-review`.
