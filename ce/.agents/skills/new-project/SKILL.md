...
name: new-project
description: Cowork onboarding - init project via init_project (Cowork variant); get_slash_command: new-project
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/new-project`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`new-project`**
...
- Retry with **`get_slash_command`** and `new-project`.
