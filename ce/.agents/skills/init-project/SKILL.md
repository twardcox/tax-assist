...
name: init-project
description: Development init via init_project; playbook ce://commands/init-project; Cowork onboarding uses /new-project
...
2. Load the canonical playbook text:
   - Call **`resources/read`** with URI **`ce://commands/init-project`** (plural `commands`), **or**
   - Call **`get_slash_command`** with **`command_name`:** **`init-project`**
...
- Retry with **`get_slash_command`** and `init-project`.
