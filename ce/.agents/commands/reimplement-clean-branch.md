...
Commit 6: "Add tests for auth flow"
```

### Rules

- Never add yourself as an author or contributor
- Never include AI-attribution footers (e.g. "Generated with [AI tool]" or "Co-Authored-By" lines) in commits
- The end state of the clean branch must be identical to the source branch
- Each commit should build/compile (use --no-verify if needed for intermediate)
